...
### Local preview opens blank, Kagi, or another external page

Use this when an Evidence/localhost preview should open but `browser-surfaces` shows an empty URL, `New tab`, Kagi, or another unrelated page.
...
If the surface URL is blank or external, navigate that surface explicitly:

```bash
PREVIEW_URL="<output from workspace-helper preview-url>"
cmux browser <surface> goto "$PREVIEW_URL"
...
Only take screenshots after `get url` matches the preview URL and browser errors have been checked. If explicit navigation still fails, switch to `cmux-diagnostics` and report the expected preview URL, actual surface URL, and any console/errors output.
